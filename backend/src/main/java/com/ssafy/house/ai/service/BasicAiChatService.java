...
public class BasicAiChatService implements AiChatService {
    ...
    public CustomChatResponseDto userControlledChat(String userInput) {
        ...
        while (chatResponse.hasToolCalls()) {
            ...
            // ───── 툴 결과 조회 ─────
            // conversationHireplace-with-user-password() 맨 마지막에 ToolResponseMessage가 담겨 있으니 꺼냅니다.
            ...
            String raw = toolResponseMessage.getResponses().get(0).responseData();
            logger.warn("[툴 호출 결과 원본]: " + raw);
            chatMemory.add(convoId, new UserMessage(raw));
            // ─────────────────────────
