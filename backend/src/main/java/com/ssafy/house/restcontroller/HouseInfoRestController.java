...
public class HouseInfoRestController {
    ...
    @PostMapping("/batch")
    public ResponseEntity<List<HouseInfoSimple>> getBySeqList(
            @RequestBody List<String> seqs) throws Exception {
        List<HouseInfoSimple> list = houseInfoService.getHouseInfoBySeqList(seqs);
        return ResponseEntity.ok(list);
    }
    ...
    public ResponseEntity<HouseInfoFull> getFullInfo(@PathVariable String aptSeq) throws Exception {
        HouseInfoFull full = houseInfoService.getHouseInfoFull(aptSeq);
        return full != null
    ...
    // !필터 검색
    @GetMapping("/filter")
    public ResponseEntity<List<HouseInfoSimple>> searchFilter(
        ...
        // HouseInfoSimple DTO 리스트를 반환하도록 시그니처를 변경하세요.
        List<HouseInfoSimple> list = houseInfoService.searchFilter(sido, gugun, dong, aptNm);
        return ResponseEntity.ok(list);
    }
}
