  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden relative flex flex-col">
        ...
        <div class="flex justify-between items-center">
          ...
          <nav class="flex space-x-1 bg-gray-100 p-1 rounded-lg">
            <button v-for="tab in tabs" :key="tab" @click="activeTab = tab"
              ...
              :class="activeTab === tab ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'">
              {{ tabNames[tab] }}
            </button>
          </nav>
      ...
      <div class="flex-1 overflow-auto p-8 bg-gray-50">
        ...
        <!-- 그리드/리스트 -->
        <div v-else :class="view === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'
          ">
          <div v-for="item in currentList" :key="item.aptSeq || item.id"
            ...
            <div :class="view === 'grid' ? 'flex flex-col h-full' : 'flex'">
              <div :class="view === 'grid' ? 'h-48 overflow-hidden' : 'w-1/3 h-32 overflow-hidden'" class="relative">
        ...
        <!-- 데이터 없을 때 -->
        <div v-if="!loading && currentList.length === 0" class="flex flex-col items-center justify-center py-16">
      ...
      <div class="px-8 py-4 bg-white border-t flex justify-between items-center">
        <p class="text-sm text-gray-500">
          <span class="font-medium text-emerald-600">{{ currentList.length }}</span>개의 매물이 표시됨
        </p>
      </div>
    </div>
  </div>
