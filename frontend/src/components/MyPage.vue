            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <div class="p-5 flex-1 flex flex-col">
                  ...
                  <div
                    class="mt-auto pt-3 border-t border-gray-100 flex justify-between items-center"
                  >
                    <button
...
onMounted(async () => {
  ...
  try {
    ...
    // 백엔드에서 리스트 가져오기
    const res = await axios.get(`/v1/members/${mno}/favorites`)
...
async function updateProfile() {
  try {
    ...
    await axios.put('/v1/members', payload)

    // 성공 메시지 표시
    const successMessage = document.createElement('div')
    successMessage.className =
      'fixed top-4 right-4 bg-emerald-100 border-l-4 border-emerald-500 text-emerald-700 p-4 rounded shadow-md z-50'
