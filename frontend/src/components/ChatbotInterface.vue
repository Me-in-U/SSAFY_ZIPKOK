    ...
    <div class="h-full bg-white rounded-lg shadow flex flex-col">
        ...
        <div class="p-4 border-b flex flex-row items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <button class="p-1 text-gray-500 hover:text-gray-700 rounded-full hover:bg-gray-100" @click="clearChat">
                ...
                <button @click="isMinimized = !isMinimized">
                    <svg v-if="isMinimized" xmlns="http://www.w3.org/2000/svg" width="18" height="18"
        ...
        <div v-if="!isMinimized" class="flex-1 flex flex-col">
            ...
            <div class="flex-1 overflow-auto p-4 space-y-4" ref="messagesContainer">
                <div v-for="(message, index) in messages" :key="index" class="flex"
                    ...
                    <div class="max-w-[80%] rounded-lg px-4 py-2" :class="{
                        'bg-emerald-600 text-white': message.sender === 'user',
                        'bg-gray-100 text-gray-800': message.sender === 'bot'
                    }">
                        <div class="flex items-center gap-2 mb-1">
                            <svg v-if="message.sender === 'bot'" xmlns="http://www.w3.org/2000/svg" width="16"
                        ...
                        <p>{{ message.content }}</p>
                        <div v-if="message.sender === 'bot' && message.options && message.options.length > 0"
            ...
            <div class="p-4 border-t">
                <div class="flex w-full items-center gap-2">
                    ...
                    <button class="p-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition"
                        @click="sendMessage">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
const botResponses = [
    {
        keywords: ["강남", "시세"],
        response: "현재 서울 강남 지역의 아파트 시세는 평균 3.2% 상승했습니다. 특히 압구정, 청담 지역은 재건축 이슈로 5% 이상 상승했습니다. 더 자세한 정보가 필요하신가요?",
        options: ["재건축 단지 정보 알려줘", "강남 투자 추천 매물 보여줘"]
    },
    {
        keywords: ["투자", "추천", "지역"],
        response: "현재 투자 수익률이 높은 지역으로는 판교, 송파, 하남 등이 있습니다. 특히 교통 인프라 개발이 예정된 하남시는 향후 가치 상승이 기대됩니다. 특정 지역에 관심이 있으신가요?",
        options: ["판교 매물 보여줘", "하남 개발 계획 자세히"]
    },
    {
        keywords: ["전세", "월세"],
        response: "전세와 월세는 투자 목적에 따라 선택이 달라집니다. 전세는 초기 자본이 많이 필요하지만 안정적이고, 월세는 현금 흐름이 중요한 투자자에게 적합합니다. 현재 시장에서는 월세 수익률이 평균 4~5%로 전세 대비 유리한 편입니다.",
        options: ["월세 수익률 높은 매물 추천", "전세 대출 조건 알려줘"]
    },
    {
        keywords: ["초보", "팁", "조언"],
        response: "부동산 투자 초보자라면 다음 사항을 고려하세요: 1) 교통과 편의시설이 좋은 입지 선택 2) 총 자산의 30% 이상 투자하지 않기 3) 대출 상환 계획 세우기 4) 세금 및 관리비 계산하기 5) 장기적 관점으로 접근하기. 더 구체적인 조언이 필요하신가요?",
        options: ["입지 선택 팁 더 알려줘", "대출 관련 조언 부탁해"]
    },
    {
        keywords: ["매수", "타이밍"],
        response: "부동산 시장은 현재 금리 인상과 정부 정책 변화로 조정기에 있습니다. 하지만 지역과 매물에 따라 상황이 다릅니다. 장기 투자 관점에서는 실수요 지역의 우량 매물은 지금도 좋은 매수 기회가 될 수 있습니다. 특정 지역에 관심이 있으신가요?",
        options: ["실수요 지역 추천해줘", "금리 전망은 어때?"]
    },
    {
        keywords: ["시장", "동향"],
        response: "최근 부동산 시장은 금리 상승으로 인해 전반적으로 거래량이 감소했습니다. 그러나 역세권, 학군이 좋은 지역, 재개발 예정 지역은 여전히 수요가 있습니다. 특히 서울 외곽과 신도시 지역의 가격 조정폭이 커서 투자 기회가 생기고 있습니다.",
...
function sendMessage() {
    ...
    setTimeout(() => {
        ...
        if (!foundResponse) {
            foundResponse = {
                response: "해당 질문에 대한 정보를 찾아보겠습니다. 부동산 투자와 관련하여 더 구체적인 질문이 있으시면 말씀해주세요.",
                options: ["투자 추천 매물 보여줘", "요즘 부동산 시장 어때?", "투자 수익률 높은 지역은?"]
            };
        }
