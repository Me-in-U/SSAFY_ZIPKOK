<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-[85vh] overflow-hidden relative flex flex-col">
      <!-- 헤더 -->
      <div class="flex items-center justify-between px-8 py-6 border-b border-gray-100 sticky top-0 bg-white z-10"
        ...
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">부동산
            커뮤니티</h2>
        </div>
        <button @click="close"
      ...
      <!-- 카테고리 탭 -->
      <div class="pl-8 pr-8 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white sticky z-10"
        :style="{ top: '80px' }">
      ...
      <!-- 검색 & 글쓰기 -->
      <div class="px-8 py-4 bg-white border-b border-gray-100 flex flex-wrap gap-4 items-center sticky z-10"
        :style="{ top: '140px' }">
        <div class="relative flex-1 min-w-[300px]">
          <input v-model="searchQuery" @keyup.enter="searchPosts" type="text" placeholder="궁금한 내용을 검색해보세요..."
          ...
          <svg xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5 absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" fill="none"
      ...
      <div class="flex-1 overflow-auto p-8">
        ...
        <div v-else class="space-y-4">
          <div v-for="post in posts" :key="post.id"
            class="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg hover:border-emerald-200 transition-all duration-200 cursor-pointer group"
            ...
            <div class="flex justify-between items-start mb-4">
              ...
              <div v-if="userStore.profile?.mno === post.authorId" class="flex gap-2 ml-4">
                <button @click.stop="startEditPost(post)"
      ...
      <!-- 페이지네이션 -->
      <div class="px-8 py-6 border-t border-gray-100 flex justify-center bg-gray-50">
        <div class="flex items-center gap-2">
          <button @click="currentPage > 1 && currentPage--" :disabled="currentPage === 1"
            class="px-4 py-2 rounded-lg border transition-colors"
            :class="currentPage === 1 ? 'text-gray-400 border-gray-200 cursor-not-allowed' : 'text-gray-700 border-gray-300 hover:bg-white hover:border-emerald-300'">
            이전
          </button>
          <button v-for="n in totalPages" :key="n" @click="currentPage = n"
            class="px-4 py-2 rounded-lg border transition-all" :class="currentPage === n
          ...
          <button @click="currentPage < totalPages && currentPage++" :disabled="currentPage === totalPages"
            class="px-4 py-2 rounded-lg border transition-colors"
            :class="currentPage === totalPages ? 'text-gray-400 border-gray-200 cursor-not-allowed' : 'text-gray-700 border-gray-300 hover:bg-white hover:border-emerald-300'">
            다음
          </button>
        </div>
      </div>
    ...
    <div v-if="selectedPost"
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden relative flex flex-col">
        <div class="sticky top-0 bg-white z-10 border-b border-gray-100 px-8 py-6 flex justify-between items-center">
          <div class="flex-1">
          ...
          <div class="flex items-center gap-3">
            ...
            <div v-if="userStore.profile?.mno === selectedPost.authorId" class="flex gap-2">
              <button @click="startEditPost(selectedPost)"
        ...
        <div class="flex-1 overflow-auto p-8">
          ...
          <div class="prose max-w-none mb-8">
            <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">{{ selectedPost.content }}</p>
          </div>
          ...
          <div class="border-t border-gray-100 pt-8">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6">
              ...
              <textarea v-model="commentText"
                class="w-full p-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"
                placeholder="댓글을 입력해주세요..." rows="4"></textarea>
    ...
    <div v-if="showPostForm"
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden relative flex flex-col">
        <div class="sticky top-0 bg-white z-10 border-b border-gray-100 px-8 py-6 flex justify-between items-center">
          <h3 class="text-2xl font-bold text-gray-800">{{ editingPost ? '게시글 수정' : '새 게시글 작성' }}</h3>
        ...
        <div class="flex-1 overflow-auto p-8">
          <form @submit.prevent="submitPost" class="space-y-6">
            <div>
              ...
              <select v-model="newPost.categoryId" required
                class="w-full p-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
                <option value="">카테고리를 선택해주세요</option>
                <option v-for="cat in categories.filter(c => c.id !== 'all')" :key="cat.id" :value="cat.id">{{ cat.name
                  }}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <input v-model="newPost.title" type="text" required placeholder="제목을 입력해주세요"
                class="w-full p-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
            </div>

            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-3">내용</label>
              <textarea v-model="newPost.content" rows="12" required placeholder="내용을 입력해주세요"
                class="w-full p-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"></textarea>
            </div>

            <div class="flex justify-end gap-4 pt-6 border-t border-gray-100">
              <button type="button" @click="cancelEditPost"
...
async function deleteComment(id) {
  ...
  await axios.delete(`http://localhost:8080/api/v1/community/posts/${selectedPost.value.id}/comments/${id}`)
  await openPostDetail(selectedPost.value)
}
...
async function submitPost() {
  if (editingPost.value) {
    await axios.put(`http://localhost:8080/api/v1/community/posts/${editingPost.value.id}`, newPost.value)
  } else {
...
function getCategoryClass(id) {
  switch (id) {
    case 'invest': return 'bg-blue-100 text-blue-700 border border-blue-200'
    case 'review': return 'bg-purple-100 text-purple-700 border border-purple-200'
    case 'qa': return 'bg-yellow-100 text-yellow-700 border border-yellow-200'
    case 'region': return 'bg-green-100 text-green-700 border border-green-200'
    case 'free': return 'bg-pink-100 text-pink-700 border border-pink-200'
...
function formatDate(dt) {
  ...
  const [datePart, timePart = ''] = dt.includes('T')
    ? dt.split('T')
    : dt.split(' ')
