...
async function fetchPosts() {
  loading.value = true
  try {
    const res = await axios.get('/v1/community/posts', {
      params: {
...
async function openPostDetail(post) {
  const { data: p } = await axios.get(`/v1/community/posts/${post.id}`)
...
async function addComment() {
  ...
  await axios.post(
    `/v1/community/posts/${selectedPost.value.id}/comments`,
...
async function saveEditComment(id) {
  ...
  await axios.put(
    `/v1/community/posts/${selectedPost.value.id}/comments/${id}`,
...
async function submitPost() {
  if (editingPost.value) {
    await axios.put(
      `/v1/community/posts/${editingPost.value.id}`,
      newPost.value,
    )
  } else {
    await axios.post('/v1/community/posts', newPost.value)
  }
  cancelEditPost()
  await fetchPosts()
}
async function deletePost(id) {
  ...
  try {
    await axios.delete(`/v1/community/posts/${id}`)
    // 상세 모달이 열려 있었다면 닫아주기
